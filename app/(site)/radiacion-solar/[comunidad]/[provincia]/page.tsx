...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    return {
        title: `Radiación Solar en ${provincia.replace(/-/g, " ")}`,
        description: `Consulta el nivel de radiación solar en los municipios de la provincia de ${provincia.replace(/-/g, " ")}.`,
    };
}

export default function RadiacionSolarProvinciaPage({ params }: Props) {
            ...
            <div className="bg-slate-900 text-white">
                <div className="mx-auto max-w-5xl px-4 py-8">
                    <nav className="text-xs text-slate-400 flex gap-1.5 items-center inline-flex mb-4">
                        ...
                        <span>›</span>
                        <a href={`/radiacion-solar/${comunidad}`} className="hover:text-white transition-colors capitalize">{comunidad.replace(/-/g, " ")}</a>
                        <span>›</span>
                    ...
                    <h1 className="text-3xl font-bold leading-tight capitalize">
                        Radiación en {provincia.replace(/-/g, " ")}
                    </h1>
                </div>
            </div>
            <div className="mx-auto max-w-5xl px-4 py-8">
                <GeoDirectory
                    ...
                    baseRoute={`/radiacion-solar/${comunidad}/${provincia}`}
                />
            </div>
        </main>
    );
