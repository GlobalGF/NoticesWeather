...
export default async function NormativaSolarRootPage({ searchParams }: Props) {
  ...
  if (provincia) {
    ...
    if (!provStats) {
      return <GenericNormativaSolarPage />;
    }
    ...
    return (
      <main className="bg-slate-50 min-h-screen font-sans overflow-x-hidden">
        ...
        <div className="relative pb-24 pt-16 overflow-hidden shadow-lg">
          ...
          <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-red-500/15 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/3 pointer-events-none" />
          ...
          <div className="mx-auto max-w-4xl px-4 relative z-30 text-center">
            <div className="inline-flex items-center gap-3 mb-5 bg-white/10 backdrop-blur-lg border border-white/20 px-5 py-2.5 rounded-full">
              ...
              <p className="text-red-300 font-bold tracking-widest uppercase text-[10px]">
                Permisos y Regulaciones en {provStats.provinceName}
              </p>
            </div>

            <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-white tracking-tight leading-tight mb-5">
              Normativa Solar en <br className="hidden md:block" />
              <span className="bg-gradient-to-r from-red-300 to-rose-400 bg-clip-text text-transparent">{provStats.provinceName}</span>
            </h1>

            <p className="text-sm md:text-lg text-slate-300 max-w-2xl mx-auto font-light leading-relaxed mb-4">
              Evita sanciones. Revisa los permisos de obra, licencias y requisitos urbanísticos exigidos por los ayuntamientos de {provStats.provinceName} para instalar placas solares.
            </p>
          </div>
        </div>
        ...
        <ProvincePageClient
          hubName="Normativa Solar"
          baseRoute="/normativa-solar"
...
function GenericNormativaSolarPage() {
  return (
      ...
      <div className="bg-slate-900 text-white">
        <div className="mx-auto max-w-5xl px-4 py-8 md:py-16 text-center">
          ...
          <p className="text-slate-400 max-w-2xl mx-auto">
            Consulta la normativa vigente, exenciones de licencia de obra y regulaciones requeridas para instalar paneles solares en tu municipio.
          </p>
        </div>
      </div>
