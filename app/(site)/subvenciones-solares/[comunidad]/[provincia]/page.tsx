...
// TOP PRIORITY MUNICIPALITIES (for internal linking boost)
const PRIORITY_SLUGS = new Set([
    "selva-illes-balears", "calvia-illes-balears", "buger-illes-balears", "bunyola-illes-balears",
    "montuiri-illes-balears", "mao-illes-balears", "felanitx-illes-balears", "santa-eugenia-illes-balears",
    "santanyi-illes-balears", "llucmajor-illes-balears", "binissalem-illes-balears", "arta-illes-balears",
    "son-servera-illes-balears", "banyalbufar-illes-balears", "esporles-illes-balears",
    "blanca", "penalen", "paradas", "barracas", "dios-le-guarde", "salas", "adios",
    "granada", "el-tanque", "antigua-las-palmas", "ingenio-las-palmas", "mediana-de-aragon",
    "alcala-de-la-selva", "aranjuez", "madrid"
]);
        ...
        <main className="bg-white min-h-screen font-sans">
            ...
            <div className="mx-auto max-w-5xl px-4 pb-10 grid gap-8 md:grid-cols-3">
                <div className="md:col-span-2 space-y-8">
                    ...
                    {/* ── Priority Municipalities (Boost) ────────────────── */}
                    {statsArr.some(m => PRIORITY_SLUGS.has(m.slug)) && (
                        <section className="space-y-4">
                            <div className="flex items-center gap-2 pb-2 border-b-2 border-emerald-500">
                                <span className="p-1 bg-emerald-500 rounded text-white"><svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg></span>
                                <h2 className="text-lg font-black text-slate-800 uppercase tracking-tight">Municipios con Máximo Ahorro en {provName}</h2>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {statsArr.filter(m => PRIORITY_SLUGS.has(m.slug)).map((m: any) => {
                                    ...
                                    return (
                                        <a
                                            ...
                                            className="relative overflow-hidden group bg-emerald-50 border-2 border-emerald-100 rounded-xl p-4 hover:border-emerald-500 hover:bg-white transition-all duration-300 shadow-sm hover:shadow-md"
                                        >
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <span className="text-sm font-bold text-slate-900 group-hover:text-emerald-700">{m.municipio}</span>
                                                    <p className="text-[10px] text-emerald-600 font-bold uppercase mt-1">Alta Rentabilidad PVGIS</p>
                                                </div>
